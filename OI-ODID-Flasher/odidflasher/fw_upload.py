...
def wait_for_bootloader_port(timeout: float = 30.0, log=print) -> str | None:
    """Scan for an ArduPilot bootloader and return its COM port."""
    up_mod = _load_uploader()
    log("Looking for the ODID bootloader serial port ...")
    deadline = time.time() + timeout
    while time.time() < deadline:
        for port in candidate_ports():
            up = None
            try:
                up = up_mod.uploader(port, 115200, 57600, identify_only=True)
...
def upload_firmware(port: str, apj_path: str, force: bool = False, log=print) -> None:
    ...
    try:
        ...
        if up.board_type != board_id and not force:
            raise RuntimeError(
                f"Board id mismatch: bootloader reports {up.board_type}, "
                f"firmware is {board_id}. The board may not have the ODID "
